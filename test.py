...
from functions import (create_temp_directory, generate_test_file,
                       ...
                       merge_and_sort_batches, is_adjacent_lines_are_sorted,
                       hashed, count_lines)


create_temp_directory(cfg.temp_directory)

generate_test_file(cfg.generated_file, cfg.n_random_lines,
                   cfg.max_line_length, cfg.max_buffer_length, cfg.symbols,
                   cfg.n_uniform_lines_for_each_buffer, cfg.n_buffers_per_line)

sort_by_batches(cfg.generated_file, cfg.n_buffers_per_batch,
                cfg.max_buffer_length, cfg.max_symbols_in_memory,
                cfg.temp_directory)
...
def test_equal_hashes():
    assert (hashed(mmaped, cfg.max_symbols_in_memory)
            == hashed(mmaped_sorted, cfg.max_symbols_in_memory))


def test_lines_sorted():
    assert (np.sum(is_adjacent_lines_are_sorted(mmaped_sorted,
                                                cfg.max_buffer_length))
            == count_lines(mmaped_sorted)-1)
